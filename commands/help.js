...
• Categories: factions, combat, progression, quests, items, commands
...
module.exports = {
    ...
    async handleButtonInteraction(interaction) {
        try {
            ...
            if (interaction.customId.startsWith('help_section_')) {
                ...
                if (!section) {
                    return await interaction.reply({
                        content: '❌ Invalid section requested.',
                        flags: [4096] // EPHEMERAL flag
                    });
                }
                ...
                const sectionEmbed = new EmbedBuilder()
                    .setColor('#00d4aa')
                    .setTitle(`🎮 Cross Realm Chronicles - ${section.name}`)
                    .setDescription(section.content)
                    .setFooter({ 
                        text: `Cross Realm Chronicles Help • Section ${sectionIndex + 1} of ${GETTING_STARTED_SECTIONS.length}` 
                    })
                    .setTimestamp();
            ...
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    content: '❌ An error occurred while navigating the help guide.',
                    flags: [4096] // EPHEMERAL flag
                });
            }
        }
