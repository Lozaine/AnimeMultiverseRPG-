        ...
        try {
            ...
            if (interaction.customId.startsWith('factionquest_accept_')) {
                ...
                const resultEmbed = new EmbedBuilder()
                    .setColor('#7CFC00')
                    .setTitle(`📜 Mission Complete: ${quest.title}`)
                    .setDescription(`You successfully completed your mission!`)
                    .addFields(
                        { name: '📊 Rewards Gained', value: `+${quest.rewards.xp} XP\n+${quest.rewards.coins} Coins`, inline: false }
                    );
                
                if(itemReceived) {
                    resultEmbed.addFields({ name: '🎁 Item Found!', value: `You received: **${itemReceived.name}**` });
                }
